...
<template>
  <div class="">
    ...
    <div>
        <label><input type="number">단</label>
        <button v-on:click = "resultArray">계산</button>
        <p>{{dan}}</p>
        <p v-for = "(result, index) in resultArray" :key = "index">{{ result }}</p>
    </div>
...
export default {
  ...
  methods: {
    ...
    gugudan(){
      this.resultArray = []
      for (let n = 2; n < 10; n++){
          this. resultArray.push(`${this.dan} * ${n} = ${this.dan * n}`)
      }
    }
  }
}
